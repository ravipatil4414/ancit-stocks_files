...
def get_enctoken(userid, password, twofa):
    ...
    if response.status_code != 200 or 'data' not in response.json() or 'request_id' not in response.json()['data']:
    ...
    if response.status_code != 200 or 'enctoken' not in response.cookies:
        # Print the response content for debugging
        print("2FA failed:", response.text)
        raise Exception("2FA failed, please check your TOTP or PIN")
