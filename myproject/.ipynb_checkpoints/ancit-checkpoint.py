...
try:
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS enctokens1 (
    ...
    def get_enctoken(userid, password, twofa):
        ...
        login_data = login_response.json().get('data', None)
        if not login_data:
            raise Exception("Login response did not contain expected data.")
        ...
        if response.status_code != 200:
            raise Exception(f"Two-factor authentication failed: {response.text}")
        ...
        if enctoken:
            ...
            raise Exception("Failed to retrieve enctoken. Please check your credentials and try again.")
    ...
    if __name__ == "__main__":
        ...
        # Fetch instruments and insert into database
